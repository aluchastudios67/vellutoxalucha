...
export default function ProductsCatalog() {
  ...
  const handleDelete = async (prodId: string) => {
    if (!window.confirm('Are you sure you want to delete this product? This will remove all variants and images.')) return;
    try {
  ...
  const columns = [
    {
      ...
      accessor: (p: Product) => (
        <div className="w-10 h-12 rounded bg-neutral-100 dark:bg-neutral-800 overflow-hidden relative shadow-sm">
          <img src={p.images?.[0]?.url || '/assets/images/no_image.png'} alt={p.name} className="w-full h-full object-cover" />
        </div>
      ),
      className: 'w-16',
    },
    ...
    {
      header: 'Price',
      accessor: (p: Product) => <span className="font-bold text-neutral-900 dark:text-white">{p.price} GEL</span>,
      sortable: true,
      sortKey: 'price',
    },
    {
    ...
    return (
      <AdminLayout>
        <div className="flex flex-col items-center justify-center py-40 space-y-4">
          ...
          <p className="text-xs uppercase tracking-widest font-semibold text-neutral-400">Loading Products Catalog...</p>
        </div>
      </AdminLayout>
    );
  }
    ...
    <AdminLayout>
      <div className="space-y-6">
        ...
        <div className="flex justify-between items-center border-b border-neutral-100 dark:border-neutral-800 pb-5">
          <div>
            <h2 className="text-2xl font-display font-bold text-neutral-900 dark:text-white">Product Catalog</h2>
            <p className="text-xs text-neutral-400 mt-1 uppercase tracking-wider font-semibold">Browse, search, edit, and duplicate products</p>
          </div>
          {canWrite && (
            <a
