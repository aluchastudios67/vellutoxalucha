...
export default function ShopTheLook() {
  ...
  const getItemName = (id: string, defaultName: string) => {
    if (id === 'velluto-2') {
      if (language === 'GE') return 'ბრილიანტის ყელსაბამი';
      if (language === 'RU') return 'Бриллиантовое колье Элегантность';
    }
    if (id === 'velluto-5') {
      if (language === 'GE') return 'სოლიტერის ბეჭედი';
      if (language === 'RU') return 'Кольцо с бриллиანтом Солитер';
    }
    return defaultName;
  };
  ...
  const getLocalizedNameFields = (id: string) => {
    ...
    if (id === 'velluto-5') {
      return {
        ...
        nameRu: 'Кольцо с бриллиანтом Солитер'
      };
    }
    return {};
  };
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-16 items-center">
          ...
          <div ref={revealRef} className="lg:col-span-5 space-y-6">
            ...
            <div className="space-y-4 pt-6 border-t border-neutral-800">
              {LOOK_ITEMS.map((item) => (
                <div key={item.id} className="flex items-center justify-between p-4 bg-neutral-900/50 border border-neutral-800/80 rounded-xl">
                  <div className="flex items-center gap-3">
                    <img src={item.img} alt={getItemName(item.id, item.name)} className="w-10 h-12 object-cover rounded" />
                    <div>
                      <p className="text-xs font-semibold text-white">{getItemName(item.id, item.name)}</p>
                      <p className="text-xs text-neutral-300">{item.price} {t('gel')}</p>
                    </div>
                  </div>
                  <button
                    onClick={() => addToCart({ id: item.id, name: item.name, price: item.price, img: item.img, ...getLocalizedNameFields(item.id) })}
          ...
          <div className="lg:col-span-7 relative flex justify-center">
            <div className="relative aspect-[4/5] w-full max-w-lg bg-neutral-900 rounded-3xl overflow-hidden shadow-2xl">
              <img
                ...
                alt="Model styling luxury jewelry look"
                ...
                >
                  ...
                  {activeHotspot === item.id && (
                    <div className="absolute bottom-8 left-1/2 -translate-x-1/2 z-30 w-48 bg-white text-neutral-950 p-3 rounded-xl shadow-2xl animate-fade-in text-center flex flex-col items-center border border-neutral-100">
                      <img src={item.img} alt={getItemName(item.id, item.name)} className="w-12 h-14 object-cover rounded-lg mb-2" />
                      <p className="text-[11px] font-bold text-neutral-800 line-clamp-1">{getItemName(item.id, item.name)}</p>
                      <p className="text-xs font-semibold text-neutral-900 mt-0.5">{item.price} {t('gel')}</p>
                      <button
                        onClick={() => addToCart({ id: item.id, name: item.name, price: item.price, img: item.img, ...getLocalizedNameFields(item.id) })}
